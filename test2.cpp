    ...
    for (int i = 0; i < frame->nb_samples; i++) {
        for (int ch = 0; ch < frame->ch_layout.nb_channels; ch++) {
            switch (format) {
                ...
                default:
                    // For unsupported formats, do nothing
                    break;
            }
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    output_frame->nb_samples = encoder_ctx->frame_size;
    output_frame->format = encoder_ctx->sample_fmt;
    output_frame->ch_layout = encoder_ctx->ch_layout;
    ...
    while (av_read_frame(input_format_ctx, input_packet) >= 0) {
        if (input_packet->stream_index == audio_stream_index) {
            ...
            while (avcodec_receive_frame(decoder_ctx, input_frame) == 0) {
                process_audio_frame(input_frame);

                // Resample
                swr_convert(swr_ctx, output_frame->data, output_frame->nb_samples,
